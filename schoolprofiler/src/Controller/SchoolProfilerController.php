	...
	public function content($school_type, $grade, $node_ids) {
		...
		foreach ($node_ids_array as $nid) {
			if ($node = Node::load($nid)) {
				...
				foreach ($node_ids_array as $nid) {
					if ($node = Node::load($nid)) {
						...
						if (is_array($attendence_rate_data)) {
							...
							if (isset($attendence_rate_data['SCHOOL'])) {
				...
				if (isset($attendance_rows['Attendance Rate']) && $attendance_rows['Attendance Rate']) {
				...
				if (isset($suspension_rows['Suspension Rate']) && $suspension_rows['Suspension Rate']) {
		...
		if ($school_type == 'private') {
			...
			if (isset($pub_data_avg_class_size) && $pub_data_avg_class_size == 1) {
				$table_markup_public .= '
			...
			if ($node->hasField('field_school_type') && $node->get('field_school_type')->value == 2) {
			...
			if (isset($pub_data_student_demo) && $pub_data_student_demo == 1) {
			...
			if (isset($pub_data_student_char) && $pub_data_student_char == 1) {
			...
			if (isset($pub_data_teacher_char) && $pub_data_teacher_char == 1){
				$table_markup_public .= '
			...
			if (isset($pub_data_regents_exam) && $pub_data_regents_exam == 1) {
				$table_markup_public .= '
			...
			if (isset($pub_data_hs_grad_rate) && $pub_data_hs_grad_rate  == 1){
				$table_markup_public .= '
			...
			if (isset($pub_data_students_performance) && $pub_data_students_performance  == 1){
				$table_markup_public .= '
