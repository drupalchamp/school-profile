      ...
      if ($node = Node::load($nid)) {
		...
		foreach ($grades_to_display as $grade) {
		  ...
		  foreach ($attendence_data as $ar_subject => $values) {
			...
			// Iterate over the nodes of High School Completion Rates.
			$high_school_completion = json_decode($node->hs_completion_rates ?: '[]', true);
			if (isset($high_school_completion['SCHOOL'][0])) {
				$high_school_data[$nid][] = $high_school_completion['SCHOOL'][0];
				
				// Collect all subjects
				foreach ($high_school_completion['SCHOOL'][0] as $hs_subject => $hs_value) {
					$all_subjects_hscr[$hs_subject] = $hs_subject;
				}
			} else {
				$high_school_data[$nid] = [];
			}
		...
		$table_rows_hscr = '';
		foreach ($all_subjects_hscr as $hs_subject) {
			$table_row_hscr = '<tr><td>' . Html::escape($hs_subject) . '</td>';
			foreach ($node_ids_array as $nid) {
				$hs_value = isset($high_school_data[$nid][$hs_subject]) ? $high_school_data[$nid][$hs_subject] : 'NA';
				$table_row_hscr .= '<td>' . Html::escape($hs_value) . '</td>';
			}
		
			$config = \Drupal::config('school_comparison_field_setting.settings');
			$pub_data_hscr = $config->get('data_hscr');
			if ($pub_data_hscr == 2) {
				$row = '<tr class="data-display-none"><td>High School Completion Rates</td><td>close</td></tr>';
				$table_rows_hscr .= $row;  // Concatenate the row to the string
			} else {            
				$table_row_hscr .= '<td><a href="#" class="close-row">close</a></td></tr>';  // Adding the close button to each row.
				$table_rows_hscr .= $table_row_hscr;  // Concatenate the row to the string
			}
		}
		...
		if ($school_type == 'private') {
			...
			$table_markup_public .= '<h2>TODO:  The percent of students who met (level 3 or higher) or exceeded (level 4) state learning standards on the assessments in the following core subject areas</h2>' ; 
